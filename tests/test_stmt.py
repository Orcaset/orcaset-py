...
def test_stmt_total_uses_real_series_and_nests_children():
    revenue = Series(
        "Revenue",
        lambda: [(Period(date(2025, 1, 1), date(2025, 2, 1)), 100.0)],
        exact,
    )
    costs = Series(
        "Costs",
        lambda: [(Period(date(2025, 1, 1), date(2025, 2, 1)), -40.0)],
        exact,
    )
    income = MapNSeries(
        "Income",
...
def test_stmt_period_query_evaluates_date_series_at_period_boundaries():
    cash = Series("Cash Flow", list, exact)
    balance = Series(
        "Balance",
        lambda: [
            (date(2025, 1, 1), 10.0),
...
def test_stmt_date_query_evaluates_dates_and_returns_none_for_period_series():
    revenue = Series("Revenue", list, exact)
    balance = Series(
        "Balance",
        lambda: [
            (date(2025, 1, 1), 100.0),
