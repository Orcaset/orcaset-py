...
- [`southwest-tsa/`](southwest-tsa/) — Standalone example project that inlines a TSA checkpoint scrape into a Southwest (LUV) quarterly revenue nowcast. Passenger lines for the current quarter scale last quarter by QTD checkpoint change; later quarters follow last year's seasonal QoQ. Run with `uv run python main.py` from that directory.
