...
- **Evidence lives at leaves.** Only `LeafSeries` has cells and a `select` audit view. `MapSeries` / `Map2Series` / `MapNSeries` are cell-less views whose provenance is the child query nodes. Do not synthesize a stream for a view; use `resample` / `rekey` if a derived line genuinely needs cells.
