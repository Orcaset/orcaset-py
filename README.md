...
- **Series** (`series.py`): a line item is a `Series[K, V, Q]` answering `query(q) -> F[Maybe[V]]`.
  ...
  - `map`/`map_maybe`/`map2`/`merge` build *views*: they transform and combine complete answers, so they have no cells and no convention, and their provenance is the child query nodes. Evidence lives at leaves, which is where the arithmetic on cells happens.
