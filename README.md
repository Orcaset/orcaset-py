...
- **Conventions** (`conventions.py`): standard select/reduce pairs — `flow(...)` for `Period`-keyed flows (day-count proration over partial periods, summed) and `keyed(...)` for exact-keyed lookups — plus the pieces (`clip_daily`, `exact`, `sum_cells`, `only`, `only_or`) to compose your own.
