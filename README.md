...
- **Series** (`series.py`): a line item is a `Series[K, V, Q]` answering `query(q) -> F[Maybe[V]]`.
  ...
  - `map`/`map_maybe`, `Map2Series`, and `MapNSeries` build *views*: they transform and combine complete answers, so they have no cells and no convention, and their provenance is the child query nodes. Evidence lives at leaves, which is where the arithmetic on cells happens. Construct the view classes directly — there are no `map2`/`merge` helpers.
...
- **Conventions** (`conventions.py`): select/reduce pieces (`clip_daily`, `exact`, `last`, `sum_cells`, `only`, `only_or`) to compose with `LeafSeries.from_cells` / `from_pairs`. A common flow line is `clip_daily()` with `sum_cells(0.0)` (day-count proration; an uncovered window is `0.0`); an exact-keyed line is `exact()` with `only()` (an absent key answers `MISSING`); a stock carried forward as-of a probe is `last()` with `only()`.
...
- [capex](./examples/capex.py): Capex → amort schedule per cohort → cohorts combined with `MapNSeries` and `or_else(..., 0.0)` into total amort
- [interest](./examples/interest.py): Stock balance (`date` / `exact` / `only`) and flow interest (`Period` / `clip_daily` / `sum_cells`) that cross-query each other
- [map_n_costs](./examples/map_n_costs.py): Heterogeneous cost schedules resampled onto a monthly grid, then summed with `MapNSeries`
