...
The self-reference is deferred until `revenue` exists. Each new month reads the previous month, eventually reaching explicit projections. The `Thunk` here allows the continuation node to become available before its value queries the flattened series, whose seam traversal may need that node. For continuations whose dependency reads can resolve before node construction, prefer an effectful step returning a direct value. A reference to the same output period would create a cycle.
